...
def generate_query_embeddings(texts):
    """为检索问题生成BGE-M3查询向量，返回格式与generate_embeddings一致。"""

    if not isinstance(texts, list) or not texts:
        raise ValueError("参数texts必须是包含查询文本的非空列表")

    model = get_bge_m3_ef()
    embeddings = model.encode_queries(texts)
    processed_sparse = []
    for index in range(len(texts)):
        start = embeddings["sparse"].indptr[index]
        end = embeddings["sparse"].indptr[index + 1]
        ...
        processed_sparse.append(
            dict(zip(sparse_indices, sparse_data))
        )

    return {
        "dense": [item.tolist() for item in embeddings["dense"]],
